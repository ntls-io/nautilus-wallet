...
pub fn start_pin_reset(request: &StartPinReset) -> StartPinResetResult {
    ...
    let generate_pk = || -> StartPinResetResult {
        ...
        let sgx_get_key = match rmp_serde::to_vec(&stored_wallet) {
            Ok(stored) => SgxGetKey::with_salt(
                &[stored.as_slice(), &request.client_pk.as_slice()].concat(),
...
pub fn reset_wallet_pin(request: &PinReset) -> PinResetResult {
    ...
    let mut mac = HmacSha256::new_from_slice(secret.expose_secret())
        .expect("Input slice has a fixed length of 32 bytes");
    mac.update(request.new_pin.as_bytes());
    mac.verify_slice(&request.new_pin_mac).map_or_else(
        |_| PinResetResult::InvalidAuth,
        |_| match mutate_wallet(&request.wallet_id, |mut stored| {
