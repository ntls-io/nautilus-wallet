...
typedef struct ms_wallet_operation_t {
	sgx_status_t ms_retval;
...
static sgx_status_t SGX_CDECL sgx_wallet_operation(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_wallet_operation_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_wallet_operation_t* ms = SGX_CAST(ms_wallet_operation_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	ms->ms_retval = wallet_operation((const uint8_t*)_in_sealed_request_buffer, _tmp_sealed_request_size, _in_sealed_response_buffer, _tmp_sealed_response_capacity, _in_sealed_response_used);
...
SGX_EXTERNC const struct {
	...
	4,
	{
		...
		{(void*)(uintptr_t)sgx_wallet_operation, 0, 0},
